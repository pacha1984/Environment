const employees = [
      ...
      { id: 4, name: 'Isaac Cabrera', age:42, department: 'HR', salary: 90000, spec: 'Javascript' },
...
function displayEmployees() {
    const totalEmployees = employees
        .map(employee => `<p>${employee.id}: ${employee.name} - ${employee.department} - ${employee.salary} - ${employee.spec}</p>`)
        .join('');
...
function displaySpec() {
    const specEmployees = employees.filter(employee => employee.spec === 'Javascript');
    const specEmployeesDisplay = specEmployees.map((employee, index) => `<p>${employee.id}: ${employee.name} - ${employee.department} - ${employee.salary} - ${employee.spec}</p>`).join('');
